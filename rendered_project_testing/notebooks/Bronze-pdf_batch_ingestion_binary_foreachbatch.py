# my_package/bronze_ingestion.py
...
def parse_args():
    ...
    parser.add_argument("--catalog", required=True, help="Name of the UC catalog.")
    parser.add_argument("--schema", required=True, help="Name of the UC schema.")
    parser.add_argument("--volume", required=True, help="Name of the UC volume to read PDF files from.")
    parser.add_argument("--checkpoints_volume", required=True, help="Name of the UC volume for streaming checkpoints.")
...
def run_bronze_task(
    ...
    if reset_data:
        ...
        spark._jsparkSession.sparkContext()._gateway.jvm.org.apache.hadoop.fs.FileSystem.get(
            spark._jsparkSession.sparkContext()._jsc.hadoopConfiguration()
        ).delete(
            spark._jsparkSession._jvm.org.apache.hadoop.fs.Path(checkpoint_remove_path),
            True
        )
...
def main():
    """
    Main entrypoint: parse args, create/get a SparkSession, run the bronze ingestion.
    """
    args = parse_args()
