...
LARGE_FILE_THRESHOLD = int(10) * 1024 * 1024  # MB to bytes
LARGE_FILE_PROCESSING_WORKFLOW_NAME = "Jas_Test_1_async_large_file_job"
...
def foreach_batch_function_silver(batch_df, batch_id):
    ...
    if not df_small.isEmpty():
        ...
        processed_df.write.mode("append").saveAsTable(job_config.parsed_files_table_name)
...
def main():
    ...
    args = parse_args()
    spark = SparkSession.builder.appName("SilverTask_Jas_Test_1").getOrCreate()
